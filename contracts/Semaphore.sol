...
contract Semaphore is Ownable, IncrementalQuinTree {
    ...
    mapping(uint256 => AuthRequest) public authRequestMap;
    mapping (uint256 => bool) public authTokenHistory;
    mapping(uint256 => uint256) public nullifierSecretMap;
    

    uint256 public AUTH_TOKEN_VALIDITY_PERIOD=0;

    event AuthRequestEvent(uint256 authToken);
     ...
     */
    constructor(uint8 _treeLevels, uint232 _firstExternalNullifier,address _verifier)
        IncrementalQuinTree(_treeLevels, NOTHING_UP_MY_SLEEVE_ZERO)
        Ownable()
        {
            addEn(_firstExternalNullifier, true);
            verify= IZkVerfiy(_verifier);
    }
    ...
    returns (uint256) {
        ...
        function insertIdentityWSecret(uint256 _identityCommitment,uint256 _nullfierHash,uint256 _secret) public 
    returns (uint256) {
    ...
    function preBroadcastCheck (
        ...
        return nullifierHashHistory[_nullifiersHash] == false &&
            ...
            //verify.verifyProof(a, b, c, publicSignals);
    ...
    function broadcastSignal(
        ...
        uint[2] memory c,
        uint[4] memory input
    ) public isValidSignalAndProof(_signal,_root,_nullifiersHash,_externalNullifier)
    {
        // Client contracts should be responsible for storing the signal and/or
        // emitting it as an event 
        require(verify.verifyProof(a, b, c, input),  "Semaphore: invalid proof");
...
// Store the nullifiers hash to prevent double-signalling
    ...
    /*
     * Stores the auth request.
     */
    function storeAuthRequest(
        bytes memory _signal, // not used
        uint256 _root,
        uint256 _nullifiersHash,
        uint256 _secret,
        uint[2] memory a,
    ...
    ) public 
    {
        ...
        require(nullifierSecretMap[_nullifiersHash] == _secret, "Invalid secret.");

        // Client contracts should be responsible for storing the signal and/or
        // emitting it as an event 
        require(verify.verifyProof(a, b, c, input),  "AuthRequest: invalid proof");
        
        uint256 timestamp= block.timestamp;
        uint256 _authToken =  uint256(keccak256(abi.encodePacked(timestamp,_nullifiersHash)));
        authRequestMap[_authToken].identifier=_nullifiersHash;
        authRequestMap[_authToken].validity= timestamp + AUTH_TOKEN_VALIDITY_PERIOD;

        emit AuthRequestEvent(_authToken);
    }

    // NOTE:imporovement can that 3rd party can also be registred
    function verifyAuthToken(uint256 _authToken) public view returns ( uint256 ) {
        require(authRequestMap[_authToken].expired == false,"AuthToken: expired");        
        require(authRequestMap[_authToken].validity<=block.timestamp,"AuthToken: invalid");        
        uint256 id=authRequestMap[_authToken].identifier;
        // TODO fix
        //authRequestMap[_authToken].expired=true;
        // authRequestMap[_authToken].identifier=0;
        
        return id;
    }

    function verifyThisProof(uint[2] memory a,
            uint[2][2] memory b,
            uint[2] memory c,
            uint[4] memory input
       ) public returns (bool r){
            return verify.verifyProof(a, b, c, input);
       }
    ...
    // testing functions should be removed 
    function checkIfNullifierUsed(uint256 _nullifiersHash) public view returns (bool) {
        return nullifierHashHistory[_nullifiersHash];
    }
    function getCurrentTimestamp()public view returns (uint256){
        return block.timestamp;
    }

     function verifyAuthTokenTest(uint256 _authToken) public view returns ( uint256 ) {
     //   require(authRequestMap[_authToken].expired == false,"AuthToken: expired");        
      //  require(authRequestMap[_authToken].validity<=block.timestamp,"AuthToken: invalid");        
        uint256 id=authRequestMap[_authToken].identifier;
        // TODO fix
